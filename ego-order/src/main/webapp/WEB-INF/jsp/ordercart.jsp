...
<form id="orderForm" class="hide" action="/order/success" method="post">
		<input type="hidden" name="paymentType" value="1"/>
		<c:forEach items="${orderList }" var="orderList" varStatus="status">
			<c:set var="totalPrice"  value="${ totalPrice + (orderList.tbItem.price * orderList.num)}"/>
			<input type="hidden" name="orderItems[${status.index}].itemId" value="${orderList.tbItem.id}"/>
			<input type="hidden" name="orderItems[${status.index}].num" value="${orderList.num }"/>
			<input type="hidden" name="orderItems[${status.index}].price" value="${orderList.tbItem.price}"/>
			<input type="hidden" name="orderItems[${status.index}].totalFee" value="${orderList.tbItem.price * orderList.num}"/>
			<input type="hidden" name="orderItems[${status.index}].title" value="${orderList.tbItem.title}"/>
			<input type="hidden" name="orderItems[${status.index}].picPath" value="${orderList.tbItem.images[0]}"/>
		</c:forEach>
		...
		<input type="hidden" name="orderShipping.receiverZip" value="100010"/>
</form>
...
<div class="step-tit">
	<h3>送货清单</h3>
	<div class="extra-r">
					<a href="/cart/show" id="cartRetureUrl" class="return-edit ftx-05">返回修改购物车</a>
			</div>
</div>
    ...
    <div class="goods-suit goods-last">
	 <c:forEach items="${orderList }" var="orderList">
		<div class="goods-item goods-item-extra">
			...
			<div class="p-img">
				<a target="_blank" href="/item/${orderList.tbItem.id}">
					<img src="${orderList.tbItem.images[0]}" alt="">
				</a>
			</div>
			<div class="goods-msg">
				<div class="p-name">
					<a href="/item/${orderList.tbItem.id}.html" target="_blank">
						${orderList.tbItem.title }
					</a>
				</div>
				<div class="p-price">
					...
					<strong>￥<fmt:formatNumber
							...
							minFractionDigits="2" value="${orderList.tbItem.price / 100 }" /></strong>
					<!--增加预售金额显示 end-->
					<span class="ml20"> x${orderList.num} </span>
					<span class="ml20 p-inventory" skuId="11555193">
					<c:if test="${orderList.tbItem.num>=orderList.num }">
						有货
					</c:if>
					<c:if test="${orderList.tbItem.num < orderList.num }">
